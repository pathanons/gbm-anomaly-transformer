...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot vanilla Anomaly Transformer charts per ticker")
    ...
    if not scores_path.exists():
        raise FileNotFoundError(f"Missing vanilla test scores file: {scores_path}")
    ...
    print(f"[visualize_vanilla_joint] loading scores={scores_path}")
    print(f"[visualize_vanilla_joint] tickers={scores_df['ticker'].nunique()} | rows={len(scores_df)} | output_dir={output_dir}")

    for ticker_idx, (ticker, score_df) in enumerate(scores_df.groupby("ticker"), start=1):
        print(f"[visualize_vanilla_joint] ticker {ticker_idx}/{scores_df['ticker'].nunique()} -> {ticker} | windows={len(score_df)}", flush=True)
        ohlcv_path = Path(args.data_path) / f"{ticker}_ohlcv.csv"
        label_path = Path(args.data_path) / f"{ticker}_anomaly_label.csv"
        if not ohlcv_path.exists():
            print(f"[visualize_vanilla_joint] skip missing price file: {ohlcv_path}")
            continue
        ...
        ax_price.set_title(f"{ticker} | vanilla anomaly chart")
        ax_price.grid(True, alpha=0.25)

        ax_score.plot(score_df["end_date"], score_df["score"], color="#2ca02c", linewidth=1.0, label="Vanilla anomaly score")
        ax_score.scatter(
        ...
        fig.tight_layout()
        out_path = output_dir / f"{ticker}_vanilla_anomaly_price_chart.png"
