...
def association_losses(series_list, prior_list, temperature: float):
    series_loss = 0.0
    prior_loss = 0.0
    for series, prior in zip(series_list, prior_list):
        prior_norm = prior / (torch.sum(prior, dim=-1, keepdim=True) + 1e-8)
        series_loss = series_loss + my_kl_loss(series, prior_norm.detach()) * temperature
        prior_loss = prior_loss + my_kl_loss(prior_norm, series.detach()) * temperature
    num_layers = max(1, len(prior_list))
    return series_loss / num_layers, prior_loss / num_layers


def run_epoch(model, loader, device, optimizer=None, k: float = 3.0, temperature: float = 50.0):
    ...
    with context:
        for batch_idx, batch in enumerate(loader, start=1):
            ...
            loss1 = rec_loss - k * series_loss.mean()
            loss2 = rec_loss + k * prior_loss.mean()

            if batch_idx == 1 or batch_idx == batch_total or batch_idx % max(1, batch_total // 5) == 0:
                print(
                    f"    [{phase}] batch {batch_idx}/{batch_total} | loss1={float(loss1.item()):.6f} "
                    f"| loss2={float(loss2.item()):.6f} | rec={float(rec_loss.item()):.6f} "
                    f"| series={float(series_loss.mean().item()):.6f} | prior={float(prior_loss.mean().item()):.6f}",
                    flush=True,
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a single vanilla Anomaly Transformer on all tickers jointly")
    ...
    print(f"[train_vanilla_joint] device={device}", flush=True)
    ...
    print(f"[train_vanilla_joint] windows={len(manifest)} | split_counts={split_counts}")
    print(f"[train_vanilla_joint] train={len(train_ds)} | val={len(val_ds)} | test={len(test_ds)}")
    print(f"[train_vanilla_joint] manifest={manifest_path}")
    print(f"[train_vanilla_joint] train_loader_batches={len(train_loader)} | val_loader_batches={len(val_loader)} | test_loader_batches={len(test_loader)}")
    print(f"[train_vanilla_joint] tickers={manifest['ticker'].nunique()}", flush=True)
    ...
    checkpoint_path = checkpoint_dir / "vanilla_joint.pt"

    history = []
    for epoch in range(args.epochs):
        print(f"[train_vanilla_joint] epoch {epoch + 1}/{args.epochs} starting", flush=True)
        train_loss = run_epoch(model, train_loader, device, optimizer=optimizer, k=args.k, temperature=args.temperature)
        val_loss = run_epoch(model, val_loader, device, optimizer=None, k=args.k, temperature=args.temperature)
    ...
    save_json(run_dir / "logs" / "vanilla_joint_training.json", history)
    save_json(
        run_dir / "configs" / "vanilla_joint_config.json",
        {
            **vars(args),
            "input_dim": input_dim,
