...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the vanilla pipeline from a flat YAML config")
    ...
    base_exp_name = str(config.get("exp_name", "experiment3_vanilla_joint"))
    ...
    def build_command(window_size: int, exp_name: str) -> list[str]:
        command = [
            sys.executable,
            "-u",
            str(root / "scripts" / "gbm" / "run_vanilla_joint.py"),
        ]
    ...
    print(f"[run_vanilla_from_yaml] config={config_path}")
    ...
    for command in commands:
        print("[run_vanilla_from_yaml] command:")
        print(" ".join(command))
        if args.dry_run:
            continue
